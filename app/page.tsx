    ...
    <section className="calculator" id="calculator">
      ...
      <div className="workspace">
        ...
        <aside className="results" aria-live="polite">
          ...
          {country === "nl" && <details className="payrollBreakdown" open><summary>Monthly net income breakdown <span>⌄</span></summary><div className="payrollRows"><div><span>Gross package</span><b>{euro.format(result.annualGross/12)}</b></div>{result.taxFree > 0 && <div className="muted"><span>Tax-free under 30% ruling</span><b>{euro.format(result.taxFree/12)}</b></div>}<div className="muted"><span>Taxable income</span><b>{euro.format(result.taxable/12)}</b></div><div><span>Payroll tax before credits</span><b>− {euro.format(result.payrollTax/12)}</b></div>{includeSocialSecurity && <div><span>National insurance</span><b>− {euro.format(result.socialTax/12)}</b></div>}<div className="credit"><span>General tax credit</span><b>+ {euro.format(result.generalCredit/12)}</b></div><div className="credit"><span>Labour tax credit</span><b>+ {euro.format(result.labourCredit/12)}</b></div>{holidayPayout === "may" && <><div><span>Regular-month net</span><b>{euro.format(result.monthlyNet)}</b></div><div className="credit"><span>Net holiday pay in May</span><b>+ {euro.format(result.holidayNet)}</b></div></>}<div className="breakdownTotal"><span>Annualized monthly average</span><b>{euro.format(result.averageMonthlyNet)}</b></div></div></details>}
